	...
	con.query(sql, function (err, result) {
		...
		if (result) {
			result.forEach((element) => {
				utils.push(
					new UserModel(element.nom, element.email, element.id).JSON()
				);
			});
			callback(utils);
		}
...
module.exports.get_chat = function (util1, util2, callback, error) {
	var sql =
		"SELECT chat FROM chats WHERE (util1=? and util2=?) or (util1=? and util2=?)";
...
module.exports.get_all_chats = function (id_util, callback, error) {
	var utils = [];
	var sql1 = `SELECT u.*,c.chat FROM utilisateurs u,chats c where (u.id = c.util2 and c.util1=${id_util}) or (u.id = c.util1 and c.util2= ${id_util})`;
...
function select_infos_users(id_util, nom, callback, error) {
	...
	var str = "%" + nom + "%";
	var sql1 =
		"SELECT DISTINCT u.* FROM utilisateurs u,chats c where u.id != ? and " +
...
module.exports.select_infos_users_plus_demande = function (
	id_util,
	nom,
	callback,
	error
	...
	select_infos_users(
		...
		function (res) {
			var str = "%" + nom + "%";
			var sql1 =
				"SELECT u.* FROM utilisateurs u,chats c where (u.id = c.util2 and c.util1=?)" +
...
module.exports.get_demandes_amie = function (id_util, callback) {
	var utils = [];
	var sql1 =
		'SELECT u.* FROM utilisateurs u,chats c where (u.id = c.util1 and c.util2=?) and (c.chat ="" ) ';
