...
io.on("connection", (socket) => {
	...
	new Promise((resolve, reject) => {
		...
		var list_conn_users = [];
		sqlSelect.get_all_chats(
			result,
			(amies) => {
				for (let amie of amies) {
					//console.log("*************************")
					//console.log(is_user_connected(socket, amie.id))
					//console.log("*************************")
	...
	socket.on("private message", ({ time, content, ID_dest, ID_emet }) => {
		sqlSelect.get_chat(ID_emet, ID_dest, function (result) {
			if (is_user_connected(socket, ID_dest)) {
				socket
					.to(get_socket_id(socket, ID_dest))
			...
			console.log("-----------------------------------");
			//console.log({time  : time, content : content ,ID_emet : ID_emet,ID_dest : ID_dest})
			console.log(result);
			console.log("-----------------------------------");
	...
	socket.on("File", ({ content, ID_dest, ID_emet, time }) => {
		sqlSelect.get_chat(ID_emet, ID_dest, function (result) {
			if (is_user_connected(socket, ID_dest)) {
				socket
					.to(get_socket_id(socket, ID_dest))
					.emit("File", content, time, ID_emet, ID_dest);
			}
			let cont = {
				id_personne: ID_emet,
				time: time,
		...
		sqlSelect.get_all_chats(
			...
			(amies) => {
				for (let amie of amies) {
					if (is_user_connected(socket, amie.id)) {
						socket
							.to(get_socket_id(socket, amie.id))
							.emit(
								"user disconnected",
								socket.handshake.auth.id
							);
...
app.use(
	expressJWT({
		...
		{ path: ["/api/v1/user/inscrire", "/api/v1/user/path"] }
	)
);
