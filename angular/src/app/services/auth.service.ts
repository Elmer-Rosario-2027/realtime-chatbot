...
export class AuthService {
    ...
    connexion(utilisateur: Object): Observable<ReponseInscrAuth>{
        return this.httpClient.post<ReponseInscrAuth>('http://localhost:3000/api/v1/user/auth', utilisateur)
            .pipe(
                tap( response => {
    ...
    chercherUtilisateur(id: any, user: any){
        this.httpClient.get('http://localhost:3000/api/v1/user/' + id).subscribe((response: any) => {
    ...
    refreshToken(): Observable<{accessToken: string}> {
        ...
        return this.httpClient.post<{accessToken: string}>('http://localhost:3000/api/v1/user/refresh-token',{ refreshToken }).pipe(
            tap(response => {
              this.setToken('token', response.accessToken);
            })
        );
